...
!
USE MPL_ALLOCATE_MOD, ONLY : MPL_ALLOCATE, MPL_DEALLOCATE
...
IF (ASSOCIATED(FOUBUF)) THEN
  IF (MAX(1,IBLEN) > SIZE(FOUBUF)) THEN
    CALL MPL_DEALLOCATE (FOUBUF)
    CALL MPL_ALLOCATE (FOUBUF, [MAX(1,IBLEN)])
...
ELSE
  ...
  FOUBUF(1)=0._JPRB ! to force allocation here
ENDIF
IF (ASSOCIATED(FOUBUF_IN)) THEN
  IF (MAX(1,IBLEN) > SIZE(FOUBUF_IN)) THEN
    CALL MPL_DEALLOCATE(FOUBUF_IN)
    CALL MPL_ALLOCATE(FOUBUF_IN, [MAX(1,IBLEN)])
