...
CONTAINS
SUBROUTINE LTINV1_CTL(KF_OUT_LT,YDGP,YDSP,FSPGL_PROC)
...
YLSP_VOR  = GREP_S2S (YDSP, KTYPE=NTYPE_VOR)
YLSP_DIV  = GREP_S2S (YDSP, KTYPE=NTYPE_DIV)
YLSP_U    = GREP_S2S (YDSP, KTYPE=NTYPE_U)
YLSP_V    = GREP_S2S (YDSP, KTYPE=NTYPE_V)
YLSP_S    = GREP_S2S (YDSP, KTYPE=NTYPE_S)
YLSP_S_NS = GREP_S2S (YDSP, KTYPE=IOR (NTYPE_S, NTYPE_NS))

!     ------------------------------------------------------------------

CALL GSTATS(102,0)
ILEI2 = 8*SIZE (YLSP_VOR) + 2*SIZE (YLSP_S) + 2*SIZE (YLSP_S_NS)
IDIM1 = 2*KF_OUT_LT
...
IF(KF_OUT_LT > 0) THEN
  ...
  DO JM=1,D%NUMP
    IM = D%MYMS(JM)
    CALL LTINV1(IM,JM,KF_OUT_LT,ILEI2,IDIM1,&
     & YDSP,YLSP_VOR,YLSP_DIV,YLSP_U, &
     & YLSP_V,YLSP_S,YLSP_S_NS,FSPGL_PROC)
