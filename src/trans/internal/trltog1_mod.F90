...
PUBLIC TRLTOG1
PRIVATE TRLTOG1_PROLOG, TRLTOG1_COMM, TRLTOG1_COMM_HEAP, TRLTOG1_COMM_STACK, TRLTOG1_CTX
...
IF (NSTACK_MEMORY_TR==0) THEN
  CALL TRLTOG1_COMM_HEAP(PGLAT,KF_FS,KF_GP,YLCTX,YDGP)
ELSE
  CALL TRLTOG1_COMM_STACK(PGLAT,KF_FS,KF_GP,YLCTX,YDGP)
ENDIF
...
IF (NSTACK_MEMORY_TR==0) THEN
  ALLOCATE(YDCTX%ZCOMBUFS(-1:YDCTX%ISENDCOUNT,YDCTX%INSEND))
...
! an OPEN-MP loop, that would cause a threads synchronization lock :
  IF (YDCTX%INSEND > 0 .AND. YDCTX%ISENDCOUNT >=-1) YDCTX%ZCOMBUFS(-1,1)=HUGE(1._JPRB)
  ALLOCATE(YDCTX%ZCOMBUFR(-1:YDCTX%IRECVCOUNT,YDCTX%INRECV))
ENDIF

END SUBROUTINE TRLTOG1_PROLOG

SUBROUTINE TRLTOG1_COMM_HEAP(PGLAT,KF_FS,KF_GP,YDCTX,YDGP)
...
CALL TRLTOG1_COMM(PGLAT,KF_FS,KF_GP,YDCTX,YDCTX%ZCOMBUFS,YDCTX%ZCOMBUFR,YDGP)

END SUBROUTINE TRLTOG1_COMM_HEAP

SUBROUTINE TRLTOG1_COMM_STACK(PGLAT,KF_FS,KF_GP,YDCTX,YDGP)
...
REAL(KIND=JPRB) :: ZCOMBUFS(-1:YDCTX%ISENDCOUNT,YDCTX%INSEND)
REAL(KIND=JPRB) :: ZCOMBUFR(-1:YDCTX%IRECVCOUNT,YDCTX%INRECV)

CALL TRLTOG1_COMM(PGLAT,KF_FS,KF_GP,YDCTX,ZCOMBUFS,ZCOMBUFR,YDGP)

END SUBROUTINE TRLTOG1_COMM_STACK

SUBROUTINE TRLTOG1_COMM(PGLAT,KF_FS,KF_GP,YDCTX,PCOMBUFS,PCOMBUFR,YDGP)
...
TYPE (TRLTOG1_CTX), INTENT(INOUT) :: YDCTX
REAL(KIND=JPRB),    INTENT(IN)    :: PGLAT(KF_FS,D%NLENGTF)
REAL(KIND=JPRB),    INTENT(INOUT) :: PCOMBUFS(-1:YDCTX%ISENDCOUNT,YDCTX%INSEND)
REAL(KIND=JPRB),    INTENT(INOUT) :: PCOMBUFR(-1:YDCTX%IRECVCOUNT,YDCTX%INRECV)
TYPE(PTRG)                        :: YDGP (:)
