...
MODULE LTINV1_MOD
CONTAINS
SUBROUTINE LTINV1(KM,KMLOC,KF_OUT_LT,KF_UV,KF_SCALARS,KF_SCDERS,KLEI2,KDIM1,&
 & PSPVOR,PSPDIV,PSPSCALAR,&
 & PSPSC3A,PSPSC3B,PSPSC2 , &
 & KFLDPTRUV,KFLDPTRSC,FSPGL_PROC,YDSP,&
 & YDSP_VOR,YDSP_DIV,YDSP_U, &
 & YDSP_V,YDSP_S,YDSP_S_NS)
...
TYPE (PTRS), OPTIONAL :: YDSP_VOR  (:)
TYPE (PTRS), OPTIONAL :: YDSP_DIV  (:)
TYPE (PTRS), OPTIONAL :: YDSP_U    (:)
TYPE (PTRS), OPTIONAL :: YDSP_V    (:)
TYPE (PTRS), OPTIONAL :: YDSP_S    (:)
TYPE (PTRS), OPTIONAL :: YDSP_S_NS (:)
...
!WRITE(CLHOOK,FMT='(A,I4.4)') 'LTINV1_',KM
...
IF(KF_SCALARS > 0)THEN
  IF(PRESENT(PSPSCALAR)) THEN
    ...
    IF (.NOT. LLSP) CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSCALAR(:,:),KF_SCALARS,KFLDPTRSC)
  ELSE

    IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
      IFIRST = ILAST+1
      ILAST  = IFIRST-1+2*NF_SC2
      IF (.NOT. LLSP) CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSC2(:,:),NF_SC2)
    ENDIF
    IF(PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
      ...
      DO J3=1,IDIM3
        ...
        IF (.NOT. LLSP) CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSC3A(:,:,J3),IDIM1)
      ENDDO
    ENDIF
    IF(PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
      ...
      DO J3=1,IDIM3
        IFIRST = ILAST+1
        ILAST  = IFIRST-1+2*IDIM1
        IF (.NOT. LLSP) CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSC3B(:,:,J3),IDIM1)
      ENDDO
    ENDIF
  ENDIF
  IF(ILAST /= 8*KF_UV+2*KF_SCALARS) THEN
    WRITE(0,*) 'LTINV1:KF_UV,KF_SCALARS,ILAST ',KF_UV,KF_SCALARS,ILAST
    CALL ABORT_TRANS('LTINV1_MOD:ILAST /= 8*KF_UV+2*KF_SCALARS')
  ENDIF
ENDIF
...
IF (KF_SCDERS > 0) THEN
  ...
  IF (.NOT. LLSP) CALL SPNSDE(KM,KF_SCALARS,ZEPSNM,ZIA(:,ISL:ISU),ZIA(:,IDL:IDU))
ENDIF
