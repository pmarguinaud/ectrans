...
IF (KF_UV > 0) THEN
  ...
  IF (.NOT. LLSP) THEN
  CALL PRFI1B(KM,ZIA(:,IVORL:IVORU),PSPVOR,KF_UV,KFLDPTRUV,YDSP((IVORL-1)/2+1:IVORU/2))
  CALL PRFI1B(KM,ZIA(:,IDIVL:IDIVU),PSPDIV,KF_UV,KFLDPTRUV,YDSP((IDIVL-1)/2+1:IDIVU/2))
  ENDIF
...
IF(KF_SCALARS > 0)THEN
  ...
  ELSE
    ...
    IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
      ...
      IF (.NOT. LLSP) CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSC2(:,:),NF_SC2,YDSP=YDSP((IFIRST-1)/2+1:ILAST/2))
    ENDIF
    IF(PRESENT(PSPSC3A) .AND. NF_SC3A > 0) THEN
      ...
      DO J3=1,IDIM3
        ...
        IF (.NOT. LLSP) CALL PRFI1B(KM,ZIA(:,IFIRST:ILAST),PSPSC3A(:,:,J3),IDIM1,YDSP=YDSP((IFIRST-1)/2+1:ILAST/2))
