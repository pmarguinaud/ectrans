...
CONTAINS
SUBROUTINE FTINV1_CTL(KF_UV_G,KF_SCALARS_G,&
...
TYPE (PTR1D), ALLOCATABLE :: YLGP_VOR    (:)
TYPE (PTR1D), ALLOCATABLE :: YLGP_DIV    (:)
TYPE (PTR1D), ALLOCATABLE :: YLGP_U      (:)
TYPE (PTR1D), ALLOCATABLE :: YLGP_V      (:)
TYPE (PTR1D), ALLOCATABLE :: YLGP_U_EW   (:)
TYPE (PTR1D), ALLOCATABLE :: YLGP_V_EW   (:)
TYPE (PTR1D), ALLOCATABLE :: YLGP_S      (:)
TYPE (PTR1D), ALLOCATABLE :: YLGP_S_EW   (:)
TYPE (PTR1D), ALLOCATABLE :: YLGP_S_NS   (:)
...
YLGP_VOR  = GREP (NPTR_VOR , IOFF)
YLGP_DIV  = GREP (NPTR_DIV , IOFF)
YLGP_U    = GREP (NPTR_U   , IOFF)
YLGP_V    = GREP (NPTR_V   , IOFF)
YLGP_S    = GREP (NPTR_S   , IOFF)
YLGP_U_EW = GREP (NPTR_U_EW, IOFF)
YLGP_V_EW = GREP (NPTR_V_EW, IOFF)
YLGP_S_EW = GREP (NPTR_S_EW, IOFF)
YLGP_S_NS = GREP (NPTR_S_NS, IOFF)

IF (IOFF /= SIZE (ZGTF, 1)) STOP 1
...
ELSE
  ...
  IF (IOFF > 0 .AND. IOFF /= KF_SCALARS_G ) THEN
    ...
    CALL ABORT_TRANS('FTINV1_CTL_MOD:IOFF /= KF_SCALARS_G')
  ENDIF
ENDIF
...
IF (ANY (YDGP%IVSET /= IVSET)) STOP 1

CALL GSTATS(157,0)
CALL TRLTOG1(ZGTF,KF_FS,KF_GP,KF_SCALARS_G,IVSET,KPTRGP,&
 &PGP,PGPUV,PGP3A,PGP3B,PGP2,YDGP)
CALL GSTATS(157,1)
...
FUNCTION GREP (KIND, KOFF) RESULT (YLGP)

TYPE (PTR1D), ALLOCATABLE :: YLGP (:)
INTEGER :: KIND, KOFF

INTEGER :: ICOUNT, J

ICOUNT = COUNT ((YDGP%KIND == KIND) .AND. (MYSETV == YDGP%IVSET))
...
DO J = 1, SIZE (YDGP)
  IF ((YDGP (J)%KIND == KIND) .AND. (YDGP (J)%IVSET == MYSETV)) THEN
    ICOUNT = ICOUNT + 1
    KOFF = KOFF + 1
    YLGP (ICOUNT)%P => ZGTF (KOFF, :)
    YLGP (ICOUNT)%KIND = KIND
  ENDIF
ENDDO
