...
INTEGER :: JPASS
TYPE (PTR2D), ALLOCATABLE :: YLGP (:)

TYPE (PTR2D), ALLOCATABLE :: YLGP_VOR  (:)
TYPE (PTR2D), ALLOCATABLE :: YLGP_DIV  (:)
TYPE (PTR2D), ALLOCATABLE :: YLGP_U    (:)
TYPE (PTR2D), ALLOCATABLE :: YLGP_V    (:)
TYPE (PTR2D), ALLOCATABLE :: YLGP_S    (:)
TYPE (PTR2D), ALLOCATABLE :: YLGP_U_EW (:)
TYPE (PTR2D), ALLOCATABLE :: YLGP_V_EW (:)
TYPE (PTR2D), ALLOCATABLE :: YLGP_S_EW (:)
TYPE (PTR2D), ALLOCATABLE :: YLGP_S_NS (:)

INTEGER (KIND=JPIM) :: ICOUNT_VOR  
INTEGER (KIND=JPIM) :: ICOUNT_DIV  
INTEGER (KIND=JPIM) :: ICOUNT_U    
INTEGER (KIND=JPIM) :: ICOUNT_V    
INTEGER (KIND=JPIM) :: ICOUNT_S    
INTEGER (KIND=JPIM) :: ICOUNT_U_EW 
INTEGER (KIND=JPIM) :: ICOUNT_V_EW 
INTEGER (KIND=JPIM) :: ICOUNT_S_EW 
INTEGER (KIND=JPIM) :: ICOUNT_S_NS 
...
DO JPASS = 1, 2
  ...
  IF (PRESENT (KVSETUV)) THEN
    ...
    IF (LVORGP) THEN
      DO J = 1, SIZE (KVSETUV)
        CALL ASSOC (ICOUNT_VOR, YLGP_VOR, PGPUV (:,J,I+1,:), NPTR_VOR, KVSETUV (J))
      ENDDO
      I = I + 1
    ENDIF
    IF (LDIVGP) THEN
      DO J = 1, SIZE (KVSETUV)
        CALL ASSOC (ICOUNT_DIV, YLGP_DIV, PGPUV (:,J,I+1,:), NPTR_DIV, KVSETUV (J))
      ENDDO
      I = I + 1
    ENDIF
    DO J = 1, SIZE (KVSETUV)
      CALL ASSOC (ICOUNT_U, YLGP_U, PGPUV (:,J,I+1,:), NPTR_U, KVSETUV (J))
    ENDDO
    DO J = 1, SIZE (KVSETUV)
      CALL ASSOC (ICOUNT_V, YLGP_V, PGPUV (:,J,I+2,:), NPTR_V, KVSETUV (J))
    ENDDO
  ENDIF

  IF (PRESENT (KVSETSC2)) THEN
    DO J = 1, SIZE (KVSETSC2)
      CALL ASSOC (ICOUNT_S, YLGP_S, PGP2 (:,J,:), NPTR_S, KVSETSC2 (J))
    ENDDO
  ENDIF
  ...
  IF (PRESENT (KVSETSC3A)) THEN
    DO I = 1, SIZE (PSPSC3A,3)
      DO J = 1, SIZE (KVSETSC3A)
        CALL ASSOC (ICOUNT_S, YLGP_S, PGP3A (:,J,I,:), NPTR_S, KVSETSC3A (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (PRESENT (KVSETSC3B)) THEN
    DO I = 1, SIZE (PSPSC3B,3)
      DO J = 1, SIZE (KVSETSC3B)
        CALL ASSOC (ICOUNT_S, YLGP_S, PGP3B (:,J,I,:), NPTR_S, KVSETSC3B (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC2)) THEN
    DO J = SIZE (KVSETSC2)+1, 2*SIZE (KVSETSC2)
      CALL ASSOC (ICOUNT_S_NS, YLGP_S_NS, PGP2 (:,J,:), IOR (NPTR_S, NPTR_NS), KVSETSC2 (J-SIZE (KVSETSC2)))
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC3A)) THEN
    DO I = 1*SIZE (PSPSC3A,3)+1, 2*SIZE (PSPSC3A,3)
      DO J = 1, SIZE (KVSETSC3A)
        CALL ASSOC (ICOUNT_S_NS, YLGP_S_NS, PGP3A (:,J,I,:), IOR (NPTR_S, NPTR_NS), KVSETSC3A (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC3B)) THEN
    DO I = 1*SIZE (PSPSC3B,3)+1, 2*SIZE (PSPSC3B,3)
      DO J = 1, SIZE (KVSETSC3B)
        CALL ASSOC (ICOUNT_S_NS, YLGP_S_NS, PGP3B (:,J,I,:), IOR (NPTR_S, NPTR_NS), KVSETSC3B (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LUVDER .AND. PRESENT (KVSETUV)) THEN
    ...
    IF (LDIVGP) I = I + 1
    DO J = 1, SIZE (KVSETUV)
      CALL ASSOC (ICOUNT_U_EW, YLGP_U_EW, PGPUV (:,J,I+3,:), IOR (NPTR_U, NPTR_EW), KVSETUV (J))
    ENDDO
    DO J = 1, SIZE (KVSETUV)
      CALL ASSOC (ICOUNT_V_EW, YLGP_V_EW, PGPUV (:,J,I+4,:), IOR (NPTR_V, NPTR_EW), KVSETUV (J))
    ENDDO
  ENDIF
  
  IF (LSCDERS .AND. PRESENT (KVSETSC2)) THEN
    DO J = 2*SIZE (KVSETSC2)+1, 3*SIZE (KVSETSC2)
      CALL ASSOC (ICOUNT_S_EW, YLGP_S_EW, PGP2 (:,J,:), IOR (NPTR_S, NPTR_EW), KVSETSC2 (J-2*SIZE (KVSETSC2)))
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC3A)) THEN
    DO I = 2*SIZE (PSPSC3A,3)+1, 3*SIZE (PSPSC3A,3)
      DO J = 1, SIZE (KVSETSC3A)
        CALL ASSOC (ICOUNT_S_EW, YLGP_S_EW, PGP3A (:,J,I,:), IOR (NPTR_S, NPTR_EW), KVSETSC3A (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC3B)) THEN
    DO I = 2*SIZE (PSPSC3B,3)+1, 3**SIZE (PSPSC3B,3)
      DO J = 1, SIZE (KVSETSC3B)
        CALL ASSOC (ICOUNT_S_EW, YLGP_S_EW, PGP3B (:,J,I,:), IOR (NPTR_S, NPTR_EW), KVSETSC3B (J))
      ENDDO
    ENDDO
  ENDIF

  IF (JPASS == 1) THEN
    ALLOCATE (YLGP_VOR  (ICOUNT_VOR ))
    ALLOCATE (YLGP_DIV  (ICOUNT_DIV ))
    ALLOCATE (YLGP_U    (ICOUNT_U   ))
    ALLOCATE (YLGP_V    (ICOUNT_V   ))
    ALLOCATE (YLGP_S    (ICOUNT_S   ))
    ALLOCATE (YLGP_U_EW (ICOUNT_U_EW))
    ALLOCATE (YLGP_V_EW (ICOUNT_V_EW))
    ALLOCATE (YLGP_S_EW (ICOUNT_S_EW))
    ALLOCATE (YLGP_S_NS (ICOUNT_S_NS))
  ENDIF
...
SUBROUTINE ASSOC (KCOUNT, YDGP, P, KIND, KVSET)

INTEGER (KIND=JPIM) :: KCOUNT, KIND, KVSET
TYPE (PTR2D), ALLOCATABLE :: YDGP (:)
REAL (KIND=JPRB), TARGET :: P (:,:)

KCOUNT = KCOUNT + 1

IF (ALLOCATED (YDGP)) THEN
  YDGP (KCOUNT)%P => P
  YDGP (KCOUNT)%KIND = KIND
  YDGP (KCOUNT)%IVSET = KVSET
ENDIF
