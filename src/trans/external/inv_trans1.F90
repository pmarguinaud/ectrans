...
IF(PRESENT(PGP)) THEN
  IF(PRESENT(PGPUV)) THEN
    CALL ABORT_TRANS('INV_TRANS1:PGP AND PGPUV CAN NOT BOTH BE PRESENT')
  ENDIF
  IF(PRESENT(PGP3A)) THEN
    CALL ABORT_TRANS('INV_TRANS1:PGP AND PGP3A CAN NOT BOTH BE PRESENT')
  ENDIF
  IF(PRESENT(PGP3B)) THEN
    CALL ABORT_TRANS('INV_TRANS1:PGP AND PGP3B CAN NOT BOTH BE PRESENT')
  ENDIF
  IF(PRESENT(PGP2)) THEN
    CALL ABORT_TRANS('INV_TRANS1:PGP AND PGP2 CAN NOT BOTH BE PRESENT')
  ENDIF
  IUBOUND(1:3)=UBOUND(PGP)
  IF(IUBOUND(1) < NPROMA) THEN
    WRITE(NOUT,*)'INV_TRANS1:FIRST DIM. OF PGP TOO SMALL ',IUBOUND(1),NPROMA
    CALL ABORT_TRANS('INV_TRANS1:FIRST DIMENSION OF PGP TOO SMALL ')
  ENDIF
  IF(IUBOUND(2) < IF_GP) THEN
    WRITE(NOUT,*)'INV_TRANS1:SEC. DIM. OF PGP TOO SMALL ',IUBOUND(2),IF_GP
    WRITE(NOUT,*)'IF_UV_G,IF_SCALARS_G,LSCDERS,LVORGP,LDIVGP,LUVDER ',&
     &            IF_UV_G,IF_SCALARS_G,LSCDERS,LVORGP,LDIVGP,LUVDER
    CALL ABORT_TRANS('INV_TRANS1:SECOND DIMENSION OF PGP TOO SMALL ')
  ENDIF
  IF(IUBOUND(3) < NGPBLKS) THEN
    WRITE(NOUT,*)'INV_TRANS1:THIRD DIM. OF PGP TOO SMALL ',IUBOUND(3),NGPBLKS
    CALL ABORT_TRANS('INV_TRANS1:THIRD DIMENSION OF PGP TOO SMALL ')
  ENDIF
ELSE
  IF(NPROMATR > 0 .AND. 2*IF_UV_G+IF_SCALARS_G > NPROMATR) THEN
    CALL ABORT_TRANS('INV_TRANS1:ALTERNATIVES TO USING PGP NOT SUPPORTED WITH NPROMATR>0')
  ENDIF
ENDIF
...
DO JPASS = 1, 2
  ...
  IF (PRESENT (KVSETUV)) THEN
    ...
    IF (LVORGP) THEN
      DO J = 1, SIZE (KVSETUV)
        CALL ASSOCG (PGPUV (:,J,I+1,:), NTYPE_VOR, KVSETUV (J))
      ENDDO
      I = I + 1
    ENDIF
    IF (LDIVGP) THEN
      DO J = 1, SIZE (KVSETUV)
        CALL ASSOCG (PGPUV (:,J,I+1,:), NTYPE_DIV, KVSETUV (J))
      ENDDO
      I = I + 1
    ENDIF
    DO J = 1, SIZE (KVSETUV)
      CALL ASSOCG (PGPUV (:,J,I+1,:), NTYPE_U, KVSETUV (J))
    ENDDO
    DO J = 1, SIZE (KVSETUV)
      CALL ASSOCG (PGPUV (:,J,I+2,:), NTYPE_V, KVSETUV (J))
    ENDDO
  ENDIF

  IF (PRESENT (KVSETSC2)) THEN
    DO J = 1, SIZE (KVSETSC2)
      CALL ASSOCG (PGP2 (:,J,:), NTYPE_S, KVSETSC2 (J))
    ENDDO
  ENDIF
  ...
  IF (PRESENT (KVSETSC3A)) THEN
    DO I = 1, SIZE (PSPSC3A,3)
      DO J = 1, SIZE (KVSETSC3A)
        CALL ASSOCG (PGP3A (:,J,I,:), NTYPE_S, KVSETSC3A (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (PRESENT (KVSETSC3B)) THEN
    DO I = 1, SIZE (PSPSC3B,3)
      DO J = 1, SIZE (KVSETSC3B)
        CALL ASSOCG (PGP3B (:,J,I,:), NTYPE_S, KVSETSC3B (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC2)) THEN
    DO J = SIZE (KVSETSC2)+1, 2*SIZE (KVSETSC2)
      CALL ASSOCG (PGP2 (:,J,:), IOR (NTYPE_S, NTYPE_NS), KVSETSC2 (J-SIZE (KVSETSC2)))
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC3A)) THEN
    DO I = 1*SIZE (PSPSC3A,3)+1, 2*SIZE (PSPSC3A,3)
      DO J = 1, SIZE (KVSETSC3A)
        CALL ASSOCG (PGP3A (:,J,I,:), IOR (NTYPE_S, NTYPE_NS), KVSETSC3A (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC3B)) THEN
    DO I = 1*SIZE (PSPSC3B,3)+1, 2*SIZE (PSPSC3B,3)
      DO J = 1, SIZE (KVSETSC3B)
        CALL ASSOCG (PGP3B (:,J,I,:), IOR (NTYPE_S, NTYPE_NS), KVSETSC3B (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LUVDER .AND. PRESENT (KVSETUV)) THEN
    ...
    IF (LDIVGP) I = I + 1
    DO J = 1, SIZE (KVSETUV)
      CALL ASSOCG (PGPUV (:,J,I+3,:), IOR (NTYPE_U, NTYPE_EW), KVSETUV (J))
    ENDDO
    DO J = 1, SIZE (KVSETUV)
      CALL ASSOCG (PGPUV (:,J,I+4,:), IOR (NTYPE_V, NTYPE_EW), KVSETUV (J))
    ENDDO
  ENDIF
  
  IF (LSCDERS .AND. PRESENT (KVSETSC2)) THEN
    DO J = 2*SIZE (KVSETSC2)+1, 3*SIZE (KVSETSC2)
      CALL ASSOCG (PGP2 (:,J,:), IOR (NTYPE_S, NTYPE_EW), KVSETSC2 (J-2*SIZE (KVSETSC2)))
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC3A)) THEN
    DO I = 2*SIZE (PSPSC3A,3)+1, 3*SIZE (PSPSC3A,3)
      DO J = 1, SIZE (KVSETSC3A)
        CALL ASSOCG (PGP3A (:,J,I,:), IOR (NTYPE_S, NTYPE_EW), KVSETSC3A (J))
      ENDDO
    ENDDO
  ENDIF
  ...
  IF (LSCDERS .AND. PRESENT (KVSETSC3B)) THEN
    DO I = 2*SIZE (PSPSC3B,3)+1, 3**SIZE (PSPSC3B,3)
      DO J = 1, SIZE (KVSETSC3B)
        CALL ASSOCG (PGP3B (:,J,I,:), IOR (NTYPE_S, NTYPE_EW), KVSETSC3B (J))
      ENDDO
    ENDDO
  ENDIF
...
DO J = 1, SIZE (YLGP)
  WRITE (100+MYPROC, '(I8," ",I8," ",I8," ",I8," ",I8," ",I8)') J, YLGP (J)%IRANK_G, &
       & YLGP (J)%IRANK_L, YLGP (J)%IVSET, YLGP (J)%ITYPE
ENDDO
#endif
...
SUBROUTINE ASSOCG (P, KTYPE, KVSET)
